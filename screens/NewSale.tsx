...
const NewSale: React.FC<NewSaleProps> = ({ products, customers, conversionData, currentUser, onComplete, onBudgetComplete }) => {
  ...
  const discountAmount = useMemo(() => {
    // Desconto só permitido em Dinheiro ou PIX
    if (paymentMethod === 'cartao') return 0;

    return cart.reduce((sum, item) => {
      if (item.allowDiscount === false) return sum;
      
      // Respeita o limite máximo de desconto por produto
  ...
  }, [cart, discountPercent, paymentMethod]);

  const hasDiscountRestrictions = useMemo(() => {
    return cart.some(item => item.allowDiscount === false);
  }, [cart]);
  ...
  return (
    <div className="h-full bg-brand-bg">
      {step === SaleStep.SELECT_CUSTOMER && (
            ...
            <div className="pt-2 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <button onClick={() => { setPaymentMethod('dinheiro'); }} className={`py-3 rounded-2xl border transition-all flex flex-col items-center gap-1 ${paymentMethod === 'dinheiro' ? 'bg-brand-primary text-white border-brand-primary' : 'bg-gray-50 text-gray-400'}`}>
                  <DollarSign size={18} /> <span className="text-[9px] font-black uppercase">Dinheiro</span>
                </button>
                <button onClick={() => { setPaymentMethod('cartao'); setDiscountPercent(0); }} className={`py-3 rounded-2xl border transition-all flex flex-col items-center gap-1 ${paymentMethod === 'cartao' ? 'bg-brand-primary text-white border-brand-primary' : 'bg-gray-50 text-gray-400'}`}>
                  <CreditCard size={18} /> <span className="text-[9px] font-black uppercase">Cartão</span>
                </button>
              </div>
            </div>

            {/* PAINEL DE DESCONTO - Condicionado a Dinheiro ou PIX */}
            {(paymentMethod === 'dinheiro' || paymentMethod === 'pix') && (
              <div className="bg-gray-50 p-4 rounded-3xl space-y-4 animate-in slide-in-from-top-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <TrendingDown className="text-green-600" size={18} />
                    <span className="text-[10px] font-black text-gray-800 uppercase">Desconto Especial</span>
                  </div>
                  {hasDiscountRestrictions && (
                    <div className="flex items-center gap-1 text-[8px] font-black text-red-400 uppercase">
                      <Info size={10} /> Itens fixos na lista
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  {[5, 10].map(val => (
                    <button 
                      ...
                      className={`flex-1 py-2 rounded-xl font-black text-xs transition-all ${discountPercent === val ? 'bg-brand-action text-brand-black shadow-md' : 'bg-white text-gray-400 border border-gray-100'}`}
                    >
                      {val}%
                    </button>
                  ))}
                  <div className="flex-1 relative">
                    <input 
                      type="number"
                      placeholder="Outro"
                      className={`w-full py-2 pl-3 pr-8 rounded-xl font-black text-xs outline-none transition-all ${discountPercent !== 0 && discountPercent !== 5 && discountPercent !== 10 ? 'bg-brand-action text-brand-black border-transparent' : 'bg-white text-gray-400 border border-gray-100'}`}
                      onChange={(e) => setDiscountPercent(parseFloat(e.target.value) || 0)}
                    />
                    <Percent className="absolute right-3 top-1/2 -translate-y-1/2 opacity-30" size={12} />
                  </div>
                  {discountPercent !== 0 && (
                    <button onClick={() => setDiscountPercent(0)} className="bg-red-50 text-red-500 p-2 rounded-xl">
                      <X size={16} />
                    </button>
                  )}
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button 
                  ...
                  className={`flex-1 py-4 rounded-2xl border flex flex-col items-center gap-2 transition-all ${!isDelivery ? 'bg-brand-primary text-white border-brand-primary shadow-lg' : 'bg-gray-50 text-gray-400'}`}
                >
